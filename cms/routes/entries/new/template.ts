...
export default (entry: NewEntryFormData = defaultFormData) => html`
  <section class="max-w-3xl mx-auto space-y-6">
    <header class="space-y-2">
      <h1 class="text-lg font-bold text-white">Add Entry</h1>
    </header>
    <form
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-2">
          ...
          <select
            ...
            class="w-full rounded-xl border border-dark-border bg-dark-bg/80 px-3 py-2 text-base text-white focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/50"
          >
            <option value="0" ${entry.isPrivate ? "" : "selected"}>Public</option>
            <option value="1" ${entry.isPrivate ? "selected" : ""}>Private</option>
          </select>
        </div>
